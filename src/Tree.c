...
/*name (numerote) nodes of tree*/
char **nameAll(char *prefixTip, char *prefixInternal, TypeTree *tree) {
    int n, nLeaves = 1, nInternal = 1, curr = 1, lmax;
    char **name, buffer[200];
    if(tree->size == 0)
        return NULL;
    name = (char**) malloc(tree->sizeBuf*sizeof(char*));
    ...
    for(n=0; n<tree->size; n++)
     ...
     lmax = (int) floor(log10((double)nInternal));
     for(n=0; n<tree->size; n++)
        if(tree->node[n].child != NOSUCH) {
            char *tmp = buffer;
            int i;
            if(prefixInternal != NULL)
                tmp += sprintf(tmp, "%s", prefixInternal);
            for(i=(int) floor(log10((double)curr)); i<lmax; i++)
                tmp += sprintf(tmp, "0");
            tmp += sprintf(tmp, "%d", curr);
            name[n] = (char*) malloc((strlen(buffer)+1)*sizeof(char));
...
void reindexTree(TypeTree *tree, int *index) {
	...
	for(n=0; n<tree->size; n++) {
		...
		if(tree->parent != NULL)
			newparent[index[n]] = tree->parent[n];
		if(tree->name != NULL)
...
TypeTree *stopTreeAtTime(TypeTree *tree, double time) {
	TypeTree *res;
	int n, *index, size;
	if(tree->time == NULL)
		return NULL;
	...
	for(n=0; n<tree->size; n++)
		if(tree->time[n] >= time) {
			res->time[n] = time;
			res->node[n].child = NOSUCH;
			index[size++] = n;
		}
	printf("\n\nStop Time %lf (size %d)\n", time, size);
	fprintSubtreeNewick(stdout, res->root, res);
	exit(0);
	reindexTree(res, index);
	res->size = size;
	return res;
}
		

/*Make that in all trees, which must have the same size and the same set of node identifiers, the index associate with a given identifier is the same */
void reindexTreesFromName(TypeTree **tree, int size) {
    int n, i, *index, sizeNamed, indNoNamed;
	TypeLexiTree *dict;
	
	if(size <= 0)
		return;
	index = (int*) malloc(tree[0]->size*sizeof(int));
	dict = newLexiTree();
	sizeNamed = 0;
	for(n=0; n<tree[0]->size; n++)
		if(tree[0]->name && tree[0]->name[n]) {
			if(addWordLexi(tree[0]->name[n], sizeNamed, dict)>=0)
				error("Error! duplicate identifier '%s' in tree\n", tree[0]->name[n]);
			sizeNamed++;
		}
	indNoNamed = sizeNamed;
	for(n=0; n<tree[0]->size; n++)
		if(tree[0]->name && tree[0]->name[n])
			index[n] = findWordLexi(tree[0]->name[n], dict);
		else
			index[n] = indNoNamed++;
	reindexTree(tree[0], index);
	for(i=1; i<size; i++) {
		int sizeTmp = 0;
		TypeLexiTree *dictTmp;
		if(tree[i]->size != tree[0]->size)
			error("Error! trees 1 and %d have different sizes\n", i+1);
		dictTmp = newLexiTree();
		sizeTmp = 0;
		for(n=0; n<tree[i]->size; n++)
			if(tree[i]->name && tree[i]->name[n]) {
				if(addWordLexi(tree[i]->name[n], sizeTmp, dictTmp)>=0)
					error("Error! duplicate identifier '%s' in tree %d\n", tree[i]->name[n], i+1);
				if(findWordLexi(tree[i]->name[n], dict)<0)
					error("Error! new identifier '%s' in tree %d\n", tree[i]->name[n], i+1);
				sizeTmp++;
			}
		if(sizeTmp != sizeNamed)
			error("Error! trees 1 and %d have different identifiers\n", i+1);
		for(n=0; n<tree[i]->size; n++)
			if(tree[i]->name && tree[i]->name[n])
...
/*return index i such that v in [startTime[i], startTime[i+1][*/
int getStartTimeIndex(double v, int size, double *startTime) {
	int a, b, c;
	if(v<startTime[0]) {
...
/*fill count[i] with the number of nodes in tree with time between startTime[i] and startTime[i+1] for all 0<=i<size*/
void countNodeTime(int *count, TypeTree *tree, int size, double *startTime) {
	int i, n;
	for(i=0; i<size; i++)
		count[i] = 0;
	if(tree != NULL && tree->time != NULL)
		for(n=0; n<tree->size; n++)
			if(tree->time[n] != NO_TIME) {
				int ind = getStartTimeIndex(tree->time[n], size, startTime);
				if(ind >= 0 && ind < size)
					count[ind]++;
			}
}
...
/*fill count[i] with the number of nodes in tree with time between startTime[i] and startTime[i+1] for all 0<=i<size*/
void countLineageTime(int *count, TypeTree *tree, int size, double *startTime) {
	int i, n;
	for(i=0; i<size; i++)
		count[i] = 0;
	...
	if(tree != NULL && tree->time != NULL)
		for(i=0; i<size; i++) {
			double time = startTime[i+1];
			count[i] = 1;
			for(n=0; n<tree->size; n++)
				if(tree->time[n] != NO_TIME && tree->time[n]<time) {
					if(tree->node[n].child != NOSUCH)
						count[i]++;
					else
						count[i]--;
