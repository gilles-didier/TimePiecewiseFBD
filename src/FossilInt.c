...
//printf("node %d (%s) child %d status %d fos %d time %lf\n", n, tree->name[n], tree->node[n].child, feat->status[n], feat->fossilInt[n], tree->time[n]);
		if(tree->node[n].child != NOSUCH)
			feat->status[n] = noneNodeStatus;
		else {
			if(feat->status[n] == noneNodeStatus) {
...
//printf("node %d (%s) child %d status %d fos %d \n", n, tree->name[n], tree->node[n].child, feat->status[n], feat->fossilInt[n]);
        ...
        //}
// printf("node %d child %d status %d\n", n, tree->node[n].child, feat->status[n]);
   }
//   exit(0);
}
...
TypeTimeInterval sampleFossilIntFixed(double time, void *data, void *rand_data) {
	TypeTimeInterval res;
	double length;
	length = utils_MIN(((TypeSampleIntData*)data)->max-time, utils_MIN(time-((TypeSampleIntData*)data)->min, ((TypeSampleIntData*)data)->length));
	res.inf = time-my_rand_unif_unit(rand_data)*length;
	res.sup = res.inf+length;
	return res;
}

TypeFossilIntFeature *sampleFossilIntFromFossil(TypeFossilFeature* feat, int size, void *data, TypeSampleIntFunct sampleInt, void *rand_data) {
    TypeFossilIntFeature *sample;
    int i, n, f, *tmp;
    size_t *index;

    if(feat == NULL)
        return NULL;
    sample = (TypeFossilIntFeature*) malloc(sizeof(TypeFossilIntFeature));
    sample->sizeFossil = feat->size;
    sample->sizeBufFossil = feat->size;
    sample->fossilIntList = (TypeFossilIntList*) malloc(sample->sizeFossil*sizeof(TypeFossilIntList));
    sample->fossilInt = (int*) malloc(size*sizeof(int));
    sample->status = (TypeNodeStatus*) malloc(size*sizeof(int));
    sample->endTimeTable = NULL;
    sample->endTime = NULL;
   for(i=0; i<size; i++) {
//printf("node %d status %d\n", i, feat->status[i]);
        sample->fossilInt[i] = feat->fossil[i];
        sample->status[i] = feat->status[i];
//if(sample->status[i] == unknownNodeStatus)
	//error("Node %d has unknown node status\n", i);
    }
    for(i=0; i<sample->sizeFossil; i++) {
        sample->fossilIntList[i].prec = feat->fossilList[i].prec;
//		sample->fossilList[i].time = feat->fossilIntList[i].fossilInt.inf+getUniformCont(feat->fossilIntList[i].fossilInt.sup-feat->fossilIntList[i].fossilInt.inf);
        sample->fossilIntList[i].fossilInt = sampleInt(feat->fossilList[i].time, data, rand_data);
//printf("fossil %d t %.2lf [%.2lf, %.2lf]\n", i, sample->fossilList[i].time, feat->fossilIntList[i].fossilInt.inf, feat->fossilIntList[i].fossilInt.sup);
    ...
    index = qsortindex(sample->fossilIntList, sample->sizeFossil, sizeof(TypeFossilIntList), compareFossilInt);
	for(n=0; n<size; n++)
		if(sample->fossilInt[n]>=0)
			sample->fossilInt[n] = index[sample->fossilInt[n]];

	for(f=0; f<sample->sizeFossil; f++)
		if(sample->fossilIntList[f].prec>=0)
			sample->fossilIntList[f].prec = index[sample->fossilIntList[f].prec];
    free((void*) index);
	tmp = (int*) malloc((sample->sizeFossil+1)*sizeof(int));
	for(n=0; n<size; n++) {
		int ind = 0;
		for(f=sample->fossilInt[n]; f>=0; f=sample->fossilIntList[f].prec)
			tmp[ind++] = f;
		if(ind>0) {
			qsort(tmp, ind, sizeof(int), compareInt);
			sample->fossilIntList[tmp[0]].prec = -1;
			for(f=1; f<ind; f++)
				sample->fossilIntList[tmp[f]].prec = tmp[f-1];
			sample->fossilInt[n] = tmp[ind-1];
		}
	}
	free((void*)tmp);
    return sample;
...
TypeFossilFeature *sampleFossilIntSpecial(TypeFossilIntFeature* feat, int size) {
...
TypeTimeInterval toFossilInt(char *s) {
    ...
    if(s[i] == '(' || s[i] == '[') {
        ...
        for(; s[i] != '\0' && isspace(s[i]); i++);
        if(s[i] != ':' && s[i] != ';')
            error("Missing ':' or ';' while reading a fossilInt time interval. ('%c' from '%s')\n", s[i], s);
        i++;
        for(; s[i] != '\0' && isspace(s[i]); i++);
        ind = 0;
        for(; s[i] != '\0' && s[i] != ')' && s[i] != ']' && !isspace(s[i]); i++)
...
TypeNameFossilIntTab *readFossilIntTab(FILE *f) {
    ...
    do {
        ...
        for(c = fgetc(f); c != EOF && isSeparatorFossil(c); c = fgetc(f));
        if(c == '\'' || c == '"') {
            c = fgetc(f);
        ...
        if(i>1) {
            ...
            while(c != EOF && !isline(c)) {
                for(i=0; c != EOF && !isSeparatorFossil(c) && i<MAX_NAME_SIZE; i++) {
                    bof[i] = c;
                    c = fgetc(f);
                }
                bof[i++] = '\0';
...
/*print fossilInt*/
void fprintFossilIntFeatureStop(FILE *f, TypeFossilIntFeature *feat, TypeTree *tree) {
    int i;
    for(i=0; i<tree->size; i++) {
        if(feat->fossilInt[i]>=0 || feat->status[i] == unknownNodeStatus) {
            if(tree->name != NULL && tree->name[i] != NULL)
                fprintf(f, "%s", tree->name[i]);
            else
                fprintf(f, "---");
            fprintFossilIntList(f, feat->fossilInt[i], feat->fossilIntList);
            if(feat->status[i] == unknownNodeStatus)
				fprintf(f, "\tS%lf", tree->time[i]);
            fprintf(f, "\n");
        }
    }
}
    ...
    for(n=tree->root+1; n<tree->size; n++) {
        ...
        if(nb>0) {
            ...
            if(fos->status[n] == unknownNodeStatus) {
                length += sprintf(tree->comment[n]+length," :%s=", TAG_TIME);
//                length += sprintInterval(tree->comment[n]+length, fos->endTimeTable[fos->endTime[n]]);
printf("tree->comment[%d] %p\n", n, (void*) (tree->comment[n]+length));
...
int getMinMaxFossilInt(int n, TypeFossilIntFeature *fos, double *min, double *max) {

		if(fos && fos->fossilInt[n] != NOSUCH) {
			int f;
			*min = fos->fossilIntList[fos->fossilInt[n]].fossilInt.inf;
			*max = fos->fossilIntList[fos->fossilInt[n]].fossilInt.sup;
			for(f=fos->fossilIntList[fos->fossilInt[n]].prec; f!=NOSUCH; f=fos->fossilIntList[f].prec) {
				if(fos->fossilIntList[f].fossilInt.inf<*min)
					*min = fos->fossilIntList[f].fossilInt.inf;
				if(fos->fossilIntList[f].fossilInt.sup>*max)
					*max = fos->fossilIntList[f].fossilInt.inf;
			}
			return 1;
		} else
			return 0;
...
void checkConsistencyFossilInt(TypeTree *tree,  TypeFossilIntFeature *fos) {
	int n;
	double *min, *max;
	...
	max = (double*) malloc(tree->size*sizeof(double));
	for(n=0; n<tree->size; n++) {
		if(!getMinMaxFossilInt(n, fos, &(min[n]), &(max[n]))) {
			min[n] = sqrt(-1.);
			max[n] = sqrt(-1.);
		}
	}
