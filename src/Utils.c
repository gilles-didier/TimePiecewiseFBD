...
/*return a table where the ith entry contains the entry of base of rank i*/
size_t *getTableIndex(void *base, size_t nitems, size_t size, int (*compar)(const void *, const void*)) {
    size_t *index, i;
    void **p, *data;
    if(size == 0 || nitems == 0)
        return NULL;
...
/*sort the table base and return table index where entry i contains the index in the sorted table of the entry which was at i, i.e. the rank of then entry i*/
